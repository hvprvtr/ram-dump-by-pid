...
fn run(args: Args) -> i32 {
    ...
    let device = if let Some(d) = args.device.clone() {
        d
    } else if let Some(drv) = args.driver.clone() {
        format!("PMEM://{drv}")
    } else {
        eprintln!("ошибка: укажите --driver <winpmem_x64.sys> или --device <строка>");
        return 2;
    };
...
fn backend_run(
    ...
    let vmm_dll = match &args.vmm {
        Some(v) => v.clone(),
        None => {
            eprintln!("ошибка: укажите --vmm <путь к vmm.dll>");
            return 2;
        }
    };
